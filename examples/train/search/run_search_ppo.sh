...
# Colocated PPO training+generation for Qwen2.5-3B-Instruct on SearchR1 data.
# Follow the instructions in docs/content/docs/recipes/searchr1.mdx for setup.
#
# Usage:
#   export WANDB_API_KEY=<your_key_here>
#   bash examples/train/search/run_search_ppo.sh
#
...
#   USE_CONVERSATION_MULTI_TURN=true bash examples/train/search/run_search_ppo.sh
#
#   # Step-wise with conversation multi-turn:
#   USE_CONVERSATION_MULTI_TURN=true STEP_WISE=true bash examples/train/search/run_search_ppo.sh
#
#   # Override any config via positional args (passed to Hydra):
#   bash examples/train/search/run_search_ppo.sh trainer.epochs=2 trainer.eval_interval=10

# export CUDA_VISIBLE_DEVICES=0,1,2,3
conda run -n retriever bash examples/train/search/retriever/retrieval_launch.sh 2>&1 | tee retrieval_server.log &
...
RUN_NAME="skyrl-search-3b-it-ppo"
